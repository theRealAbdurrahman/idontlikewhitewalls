  ...
  return (
    <div className="bg-[#f0efeb] flex flex-row justify-center w-full min-h-screen">
      <div className="bg-[#f0efeb] overflow-hidden w-full max-w-[390px] relative min-h-screen">
        ...
        {showHeader && (
          <div className="fixed top-0 left-0 w-full max-w-[390px] z-30 bg-[#f0efeb]">
            <Header />
        ...
        <main className={`relative ${showHeader ? (showFilterBar ? 'pt-[140px]' : 'pt-[90px]') : ''} ${showBottomNav ? 'pb-[122px]' : 'pb-4'}`}>
          ...
          {showFilterBar && (
            <div className="fixed top-[140px] left-0 w-full max-w-[390px] h-8 bg-gradient-to-b from-[#f0efeb] to-transparent pointer-events-none z-20" />
          )}
          {children}
        </main>
